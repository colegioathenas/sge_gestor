...
class SVG {
	...
	function SVG(&$mpdf) {
		...
		$this->txt_style = array (
			...
			'font-family'// pas de remplissage par defaut
						null => $mpdf->default_font,
						'font-size' => $mpdf->default_font_size, // 	****** this is pts
			'font-weight'// ****** this is pts
						null => 'normal', //	normal | bold
			'font-style'// normal | bold
						null => 'normal', //	italic | normal
			'text-anchor'// italic | normal
						null => 'start', // alignment: start, middle, end
						/* mPDF 5.0.041 */
						'fill-opacity' => 1, //	remplissage opaque par defaut
			'fill-rule'// remplissage opaque par defaut
						null => 'nonzero', //	mode de remplissage par defaut
			'stroke'// mode de remplissage par defaut
						null => 'none', //	pas de trait par defaut
			'stroke-opacity'// pas de trait par defaut
						null => 1, //	trait opaque par defaut
			'stroke-width'// trait opaque par defaut
						null => 1, //	mPDF 4.4.011
			'color'// mPDF 4.4.011
						null => ''  //	mPDF 4.4.005
			)// mPDF 4.4.005
	...
	function svgGradient($gradient_info, $attribs, $element) {
		...
		$w// Get bounding dimensions of element
		null = 100;
		$h = 100;
		$x_offset = 0;
		$y_offset = 0;
		if ($element == 'rect') {
			...
			// mPDF 5.0.039
		  if// mPDF 5.0.039
			null (is_array ( $this->pathBBox ) && $this->pathBBox [2] > 0) {
				$w = $this->pathBBox [2];
				$h = $this->pathBBox [3];
			...
			if (count ( $m [0] )) {
				for($i = 0; $i < count ( $m [0] ); $i ++) {
					...
					if ($c == 'matrix' && count ( $vv ) == 6) {
						...
						// cf svgDefineStyle()
						$transformations// cf svgDefineStyle()
						null .= sprintf ( ' %.3F %.3F %.3F %.3F %.3F %.3F cm ', $vv [0], - $vv [1], - $vv [2], $vv [3], $vv [4] * $this->kp, - $vv [5] * $this->kp );
		...
		// mPDF 5.0.039
		if// mPDF 5.0.039
		null (isset ( $gradient_info ['units'] ) && strtolower ( $gradient_info ['units'] ) == 'userspaceonuse') {
			if ($transformations) {
		...
		null = 'P'; // pad
		if// pad
		null (isset ( $gradient_info ['spread'] )) {
			if (strtolower ( $gradient_info ['spread'] ) == 'reflect') {
				$spread = 'F';
			} 			// reflect
			// reflect
			nullelse if (strtolower ( $gradient_info ['spread'] ) == 'repeat') {
				$spread = 'R';
			} // repeat
		}// repeat
		null
		...
		if ($gradient_info ['type'] == 'linear') {
			...
			$return// -y-offset
			null .= sprintf ( '%.3F 0 0 %.3F %.3F %.3F cm ', $a * $this->kp, $d * $this->kp, $e * $this->kp, $f * $this->kp );
			
			// mPDF 5.0.039
			if// mPDF 5.0.039
			null (isset ( $gradient_info ['units'] ) && strtolower ( $gradient_info ['units'] ) == 'objectboundingbox') {
				if ($transformations) {
					$return .= $transformations;
				}
			}
			...
			// mPDF 5.0.040
			if// mPDF 5.0.040
			null ($spread == 'R' || $spread == 'F') { // Repeat  /  Reflect
				$offs// Repeat / Reflect
				null = array ();
				for($i = 0; $i < $ns; $i ++) {
				...
				while ( $inside ) {
					...
					for($i = 0; $i < $ns; $i ++) {
						...
						$gradient_info// Reflect
							null ['color'] [(($ns * $gp) + $i)] = $gradient_info ['color'] [(($ns * ($gp - 1)) + ($ns - $i - 1))];
							$tmp = $gp + (1 - $offs [($ns - $i - 1)]);
							$gradient_info ['color'] [(($ns * $gp) + $i)] ['offset'] = $tmp;
						} else { // Reflect
						$gradient_info// Reflect
							null ['color'] [(($ns * $gp) + $i)] = $gradient_info ['color'] [$i];
							$tmp = $gp + $offs [$i];
							$gradient_info ['color'] [(($ns * $gp) + $i)] ['offset'] = $tmp;
						}
						// IF STILL INSIDE BOX OR STILL VALID
						// Point on axis to test
					$px1// Point on axis to test
						null = $x1 + ($x2 - $x1) * $tmp;
						$py1 = $y1 + ($y2 - $y1) * $tmp;
						// Get perpendicular axis
					$alpha// Get perpendicular axis
						null = atan2 ( $y2 - $y1, $x2 - $x1 );
						$alpha += M_PI / 2; // rotate 90 degrees
						                  // Get arbitrary point to define line perpendicular to axis
					$px2// Get arbitrary point to define line perpendicular to axis
						null = $px1 + cos ( $alpha );
						$py2 = $py1 + sin ( $alpha );
						
						$res1 = _testIntersect ( $px1, $py1, $px2, $py2, 0, 0, 0, 1 ); // $x=0 vert axis
					$res2// $x=0 vert axis
						null = _testIntersect ( $px1, $py1, $px2, $py2, 1, 0, 1, 1 ); // $x=1 vert axis
					$res3// $x=1 vert axis
						null = _testIntersect ( $px1, $py1, $px2, $py2, 0, 0, 1, 0 ); // $y=0 horiz axis
					$res4// $y=0 horiz axis
						null = _testIntersect ( $px1, $py1, $px2, $py2, 0, 1, 1, 1 ); // $y=1 horiz axis
					if// $y=1 horiz axis
						null (! $res1 && ! $res2 && ! $res3 && ! $res4) {
							$inside = false;
						}
					}
				}
				...
				while ( $inside ) {
					...
					for($i = 0; $i < $ns; $i ++) {
						if ($spread == 'F') { // Reflect
					    $newarr// Reflect
							null [$i] = $gradient_info ['color'] [($ns - $i - 1)];
							if (($gp % 2) == 1) {
								$tmp = - $gp + (1 - $offs [($ns - $i - 1)]);
								$newarr [$i] ['offset'] = $tmp;
							} else {
			...
			$stops// mPDF 4.4.007 Gradient STOPs
			null = count ( $gradient_info ['color'] );
			if ($stops < 2) {
				return '';
			}
			...
			// mPDF 5.0.042
			$range// mPDF 5.0.042
			null = $gradient_info ['color'] [count ( $gradient_info ['color'] ) - 1] ['offset'] - $gradient_info ['color'] [0] ['offset'];
			$min = $gradient_info ['color'] [0] ['offset'];
			
			for($i = 0; $i < ($stops); $i ++) {
				// mPDF 5.0.051
				if// mPDF 5.0.051
				null (! $gradient_info ['color'] [$i] ['color']) {
			...
			$this// mPDF 5.0.051
			null->mpdf_ref->gradients [$n] ['type'] = 2;
			...
			while// x1 and y1 (fx, fy) should be inside the circle defined by x0 y0 and r else error in mPDF
			null ( pow ( ($x1 - $x0), 2 ) + pow ( ($y1 - $y0), 2 ) >= pow ( $r, 2 ) ) {
				$r += 0.05;
			}
			...
			$this// mPDF 5.0.051
			null->mpdf_ref->gradients [$n] ['type'] = 3;
			$this->mpdf_ref->gradients [$n] ['fo'] = true;
	...
	function svgOffset($attribs) {
		...
		$this// save all <svg> tag attributes
		null->svg_attribs = $attribs;
		...
		$svg_h// mm (interprets numbers as pixels)
		null = $this->mpdf_ref->ConvertSize ( $attribs ['height'] ); // mm

///*
		// mPDF 5.0.005
		if// mm
		null ($this->svg_info ['w']) { // if 'w' set by viewBox
			if// if 'w' set by viewBox
			null ($svg_w) { // if width also set, use these values to determine to set size of "pixel"
				$this// if width also set, use these values to determine to set size of "pixel"
				null->kp *= ($svg_w / 0.2645) / $this->svg_info ['w'];
		...
		$this// mm->pixels
		null->svg_info ['h'] = $svg_h / 0.2645; // mm->pixels
	...
	nullfunction svg_overflow($x, $y) {
		...
		if (isset ( $this->svg_attribs ['overflow'] )) {
			if ($this->svg_attribs ['overflow'] == 'hidden') {
				...
				$svg_w// Not sure if this is supposed to strip off units, but since I dont use any I will omlt this step
				null = preg_replace ( "/([0-9\.]*)(.*)/i", "$1", $this->svg_attribs ['width'] );
				$svg_h = preg_replace ( "/([0-9\.]*)(.*)/i", "$1", $this->svg_attribs ['height'] );
				
				// $xmax = floor($this->svg_attribs['width']);
				$xmax// $xmax = floor($this->svg_attribs['width']);
				null = floor ( $svg_w );
				$xmin = 0;
				// $ymax = floor(($this->svg_attribs['height'] * -1));
				$ymax// $ymax = floor(($this->svg_attribs['height'] * -1));
				null = floor ( ($svg_h * - 1) );
				$ymin = 0;
	...
	function svgDefineStyle($critere_style) {
		...
		// TRANSFORM SCALE
		$transformations// TRANSFORM SCALE
		null = '';
			...
			if (count ( $m [0] )) {
				for($i = 0; $i < count ( $m [0] ); $i ++) {
					...
					if ($c == 'matrix' && count ( $vv ) == 6) {
						// mPDF 5.0.039
						// Note angle of rotation is reversed (from SVG to PDF), so vv[1] and vv[2] are negated
						$transformations// Note angle of rotation is reversed (from SVG to PDF), so vv[1] and vv[2] are negated
						null .= sprintf ( ' %.3F %.3F %.3F %.3F %.3F %.3F cm ', $vv [0], - $vv [1], - $vv [2], $vv [3], $vv [4] * $this->kp, - $vv [5] * $this->kp );
						
						/*
						 * // The long way of doing this?? // need to reverse angle of rotation from SVG to PDF $sx=sqrt(pow($vv[0],2)+pow($vv[2],2)); if ($vv[0] < 0) { $sx *= -1; } // change sign $sy=sqrt(pow($vv[1],2)+pow($vv[3],2)); if ($vv[3] < 0) { $sy *= -1; } // change sign // rotation angle is $t=atan2($vv[1],$vv[3]); $t=atan2(-$vv[2],$vv[0]);	// Should be the same value or skew has been applied // Reverse angle $t *= -1; // Rebuild matrix $ma = $sx * cos($t); $mb = $sy * sin($t); $mc = -$sx * sin($t); $md = $sy * cos($t); // $transformations .= sprintf(' %.3F %.3F %.3F %.3F %.3F %.3F cm ', $ma, $mb, $mc, $md, $vv[4]*$this->kp, -$vv[5]*$this->kp);
						 */
					} else if ($c == 'translate' && count ( $vv )) {
						$tm [4] = $vv [0];
						if (count ( $vv ) == 2) {
		...
		if (isset ( $critere_style ['style'] )) {
			...
			// mPDF 4.4.003
			$tmp// mPDF 4.4.003
			null = preg_replace ( "/(.*)stroke-dasharray:\s*([a-z0-9., ]*|none)(.*)/i", "$2", $critere_style ['style'] );
			if ($tmp != $critere_style ['style']) {
			...
			// mPDF 4.4.003
			$tmp// mPDF 4.4.003
			null = preg_replace ( "/(.*)stroke-dashoffset:\s*([a-z0-9.]*|none)(.*)/i", "$2", $critere_style ['style'] );
			if ($tmp != $critere_style ['style']) {
		...
		// mPDF 4.4.003
		if// mPDF 4.4.003
		null (isset ( $critere_style ['stroke-dasharray'] )) {
		...
		if// mPDF 4.4.005 Used as indirect setting for currentColor
		null (isset ( $critere_style ['color'] ) && $critere_style ['color'] != 'inherit') {
	...
	// Cette fonction ecrit le style dans le stream svg.
	nullfunction svgStyle($critere_style, $attribs, $element) {
		$path_style = '';
		if (substr_count ( $critere_style ['fill'], 'url' ) > 0) {
			//
			// couleur degrad�
			$id_gradient// couleur degrad�
			null = preg_replace ( "/url\(#([\w_]*)\)/i", "$1", $critere_style ['fill'] );
		...
		// mPDF 4.4.005 Used as indirect setting for currentColor
		nullelse if (strtolower ( $critere_style ['fill'] ) == 'currentcolor') {
			...
			if ($col) {
				...
				$path_style// CMYKa
				null .= $this->mpdf_ref->SetFColor ( $col, true ) . ' '; // mPDF 5.0.051
				$style// mPDF 5.0.051
				null .= 'F';
			}
		...
		// mPDF 5.0.040
		if// mPDF 5.0.040
		null (substr_count ( $critere_style ['stroke'], 'url' ) > 0) {
			/*
			 * // Cannot put a gradient on a "stroke" in PDF? $id_gradient = preg_replace("/url\(#([\w_]*)\)/i","$1",$critere_style['stroke']); if ($id_gradient != $critere_style['stroke']) { if (isset($this->svg_gradient[$id_gradient])) { $fill_gradient = $this->svgGradient($this->svg_gradient[$id_gradient], $attribs, $element); if ($fill_gradient) { $path_style = "q "; $w = "W"; $style .= 'D'; } } }
			 */
		} 		// mPDF 4.4.005   Used as indirect setting for currentColor
		// mPDF 4.4.005 Used as indirect setting for currentColor
		nullelse if (strtolower ( $critere_style ['stroke'] ) == 'currentcolor') {
			$col = $this->mpdf_ref->ConvertColor ( $critere_style ['color'] );
			if ($col) {
				// mPDF 5.0.051
				// mPDF 5.3.74
				...
				$path_style// CMYKa
				null .= $this->mpdf_ref->SetDColor ( $col, true ) . ' '; // mPDF 5.0.051
				$style// mPDF 5.0.051
				null .= 'D';
		...
		if// mPDF 4.4.003
			null (isset ( $critere_style ['stroke-dasharray'] )) {
			  ...
			  $arr// 5, 3, 1 => 5,3,1,5,3,1 OR 3 => 3,3
					null = '';
		...
		// mPDF 4.4.003
		if// mPDF 4.4.003
		null ($critere_style ['fill-rule'] == 'evenodd') {
			$fr = '*';
		} else {
			$fr = '';
		}
		...
		// mPDF 4.4.003
		if// mPDF 4.4.003
		null (isset ( $critere_style ['fill-opacity'] )) {
			$opacity = 1;
			...
			$path_style// mPDF 5.0.039
			null .= sprintf ( ' /GS%d gs ', $gs );
		}
		
		// mPDF 4.4.003
		if// mPDF 4.4.003
		null (isset ( $critere_style ['stroke-opacity'] )) {
			$opacity = 1;
	...
	// fonction retracant les <path />
	nullfunction svgPath($command, $arguments) {
		$path_cmd = '';
		$newsubpath = false; // mPDF 4.4.003
		                     // mPDF 5.0.039
		$minl// mPDF 5.0.039
		null = $this->pathBBox [0];
		...
		// mPDF 4.4.003
		// mPDF 4.4.003
		nullpreg_match_all ( '/[\-^]?[\d.]+(e[\-]?[\d]+){0,1}/i', $arguments, $a, PREG_SET_ORDER );
		
		//	if the command is a capital letter, the coords go absolute, otherwise relative
		if// if the command is a capital letter, the coords go absolute, otherwise relative
		null (strtolower ( $command ) == $command)
			$relative = true;
		else
			$relative = false;
		...
		switch// each command may have different needs for arguments [1 to 8]
		null (strtolower ( $command )) {
			case 'm' : // move
				for// move
				null($i = 0; $i < $ile_argumentow; $i += 2) {
					$x = $a [$i] [0];
					$y = $a [$i + 1] [0];
					if ($relative) {
						$pdfx = ($this->xbase + $x);
					...
					if// mPDF 4.4.003 Save start points of subpath
					null ($this->subPathInit) {
			...
			case 'h' : // a very simple horizontal line
				for// a very simple horizontal line
				null($i = 0; $i < $ile_argumentow; $i ++) {
					$x = ($a [$i] [0]);
					if ($relative) {
						$y = 0;
						$pdfx = ($this->xbase + $x);
			...
			case 'v' : // the simplest line, vertical
				for// the simplest line, vertical
				null($i = 0; $i < $ile_argumentow; $i ++) {
					$y = ($a [$i] [0]);
					if ($relative) {
						$x = 0;
			...
			case 's' : // bezier with first vertex equal first control
			          // mPDF 4.4.003 
			   if// mPDF 4.4.003
				null (! ($this->lastcommand == 'C' || $this->lastcommand == 'c' || $this->lastcommand == 'S' || $this->lastcommand == 's')) {
				...
				for($i = 0; $i < $ile_argumentow; $i += 4) {
					...
					$pdf_pt// mPDF 4.4.003 always relative
					null = $this->svg_overflow ( $pdfx, $pdfy );
			...
			for// bezier with second vertex equal second control
				null($i = 0; $i < $ile_argumentow; $i += 6) {
					$x1 = ($a [$i] [0]);
					$y1 = ($a [$i + 1] [0]);
					$x2 = ($a [$i + 2] [0]);
					$y2 = ($a [$i + 3] [0]);
					...
					$pdf_pt// $pdf_pt1 = $this->svg_overflow($pdfx1,$pdfy1);
					null = $this->svg_overflow ( $pdfx, $pdfy );
			...
			case 'q' : // bezier quadratic avec point de control
				for// bezier quadratic avec point de control
				null($i = 0; $i < $ile_argumentow; $i += 4) {
					$x1 = ($a [$i] [0]);
					$y1 = ($a [$i + 1] [0]);
					$x = ($a [$i + 2] [0]);
					$y = ($a [$i + 3] [0]);
			...
			case 't' : // bezier quadratic avec point de control simetrique a lancien point de control
			          // mPDF 4.4.003 
			   if// mPDF 4.4.003
				null (! ($this->lastcommand == 'Q' || $this->lastcommand == 'q' || $this->lastcommand == 'T' || $this->lastcommand == 't')) {
					$this->lastcontrolpoints = array (
							0,
							0 
					);
				...
				for($i = 0; $i < $ile_argumentow; $i += 2) {
					...
					if ($relative) {
						...
						$pdfy1// mPDF 4.4.003
						null = ($this->ybase - ($y1)); // mPDF 4.4.003
						                                 // mPDF 4.4.003 
    						$pdfx2// mPDF 4.4.003
						null = $pdfx1 + 1 / 3 * ($x);
						$pdfy2 = $pdfy1 + 1 / 3 * (- $y);
					...
					// mPDF 5.0.040
					$curves// mPDF 4.4.003 always relative
					null = array (
							$pdfx1,
							- $pdfy1,
							$pdfx2,
			...
			case 'a' : // Elliptical arc
				for// Elliptical arc
				null($i = 0; $i < $ile_argumentow; $i += 7) {
					$rx = ($a [$i] [0]);
					$ry = ($a [$i + 1] [0]);
					$angle = ($a [$i + 2] [0]); //x-axis-rotation 
					$largeArcFlag// x-axis-rotation
					null = ($a [$i + 3] [0]);
					$sweepFlag = ($a [$i + 4] [0]);
					$x2 = ($a [$i + 5] [0]);
					$y2 = ($a [$i + 6] [0]);
					...
					list// mPDF 5.0.039 // mPDF 5.0.040
					null ( $pcmd, $bounds ) = $this->Arcto ( $x1, $y1, $x2, $y2, $rx, $ry, $angle, $largeArcFlag, $sweepFlag );
		...
		$this// mPDF 4.4.003
		null->lastcommand = $command;
		// mPDF 5.0.039
		$this// mPDF 5.0.039
		null->pathBBox [0] = $minl;
	...
	if// is equivalent to omitting the elliptical arc segment entirely
		null ($x1 == $x2 && $y1 == $y2)
			return array (
					'',
					$bounds 
	...
	$sinPhi// http://www.w3.org/TR/SVG/implnote.html
		null = sin ( deg2rad ( $angle ) );
		...
		//    ry'=s*ry becomes 0 :
		$s// exactly one solution (until the ellipse is just big enough).
			null = sqrt ( 1.0 - $numerator / ($rx * $rx * $ry * $ry) );
	...
	$segments// 3. convert into cubic bezier segments <= 90deg
		null = ceil ( abs ( $dtheta / (M_PI / 2.0) ) );
		...
		$xe// a) calculate endpoint of the segment:
			null = $cosPhi * $rx * $cosTheta2 - $sinPhi * $ry * $sinTheta2 + $cx;
			$ye = $sinPhi * $rx * $cosTheta2 + $cosPhi * $ry * $sinTheta2 + $cy;
			
			// b) calculate gradients at start/end points of segment:
		$dx1// b) calculate gradients at start/end points of segment:
			null = $t * (- $cosPhi * $rx * $sinTheta1 - $sinPhi * $ry * $cosTheta1);
		...
		$coords// c) draw the cubic bezier:
			null [$i] = array (
					($x1 + $dx1),
					($y1 + $dy1),
					($xe + $dxe),
	...
	// mPDF 4.4.003
	// mPDF 4.4.003
	nullfunction ConvertSVGSizePixels($size = 5, $maxsize = 'x') {
		...
		if// Setting e.g. margin % will use maxsize (pagewidth) and em will use fontsize
		null ($maxsize == 'y') {
		...
		$fontsize// convert pixels to mm
		null = $this->mpdf_ref->FontSize;
		//Return as pixels
		$size// Return as pixels
		null = $this->mpdf_ref->ConvertSize ( $size, $maxsize, $fontsize, false ) * 1 / (25.4 / $this->mpdf_ref->dpi);
		return $size;
	}
	
	// mPDF 4.4.003
	// mPDF 4.4.003
	nullfunction ConvertSVGSizePts($size = 5) {
		// usefontsize - setfalse for e.g. margins - will ignore fontsize for % values
		// Depends of maxsize value to make % work properly. Usually maxsize == pagewidth
		// For text $maxsize = Fontsize
		// Setting e.g. margin % will use maxsize (pagewidth) and em will use fontsize
		$maxsize// Setting e.g. margin % will use maxsize (pagewidth) and em will use fontsize
		null = $this->mpdf_ref->FontSize;
		//Return as pts
		$size// Return as pts
		null = $this->mpdf_ref->ConvertSize ( $size, $maxsize, false, true ) * 72 / 25.4;
		return $size;
	}
	...
	// fonction retracant les <rect />
	nullfunction svgRect($arguments) {
		if ($arguments ['h'] == 0 || $arguments ['w'] == 0) {
			return '';
		} // mPDF 4.4.003

		$x// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['x'], 'x' ); // mPDF 4.4.003 
		$y// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['y'], 'y' ); // mPDF 4.4.003 
		$h// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['h'], 'y' ); // mPDF 4.4.003 
		$w// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['w'], 'x' ); // mPDF 4.4.003 
		$rx// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['rx'], 'x' ); // mPDF 4.4.003 
		$ry// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['ry'], 'y' ); // mPDF 4.4.003 
		...
		if ($rx == 0 and $ry == 0) {
			...
			$path_cmd// trace un rectangle sans angle arrondit
			null = sprintf ( '%.3F %.3F m ', ($x * $this->kp), - ($y * $this->kp) );
			...
			$kappa// les points de controle du bezier sont deduis grace a la constante kappa
			null = 4 * (sqrt ( 2 ) - 1) / 3;
	...
	// sont deduis grace a la constante kappa * rayon
	nullfunction svgEllipse($arguments) {
		...
		$cy// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['cy'], 'y' ); // mPDF 4.4.003 
		$rx// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['rx'], 'x' ); // mPDF 4.4.003 
		$ry// mPDF 4.4.003
		null = $this->ConvertSVGSizePixels ( $arguments ['ry'], 'y' ); // mPDF 4.4.003 
	...
	// fonction retracant les <polyline /> et les <line />
	nullfunction svgPolyline($arguments, $ispolyline = true) {
		if ($ispolyline) {
			$xbase = $arguments [0];
			$ybase = - $arguments [1];
		} else {
			...
			$xbase// mPDF 4.4.003 Zero length line
			null = $this->ConvertSVGSizePixels ( $arguments [0], 'x' ); // mPDF 4.4.003 
			$ybase// mPDF 4.4.003
			null = - $this->ConvertSVGSizePixels ( $arguments [1], 'y' ); // mPDF 4.4.003 
		}// mPDF 4.4.003
		null
		...
		for($i = 2; $i < count ( $arguments ); $i += 2) {
			if ($ispolyline) {
				...
				$tmp_y// mPDF 4.4.003
				null = - $this->ConvertSVGSizePixels ( $arguments [($i + 1)], 'y' ); // mPDF 4.4.003 
			}// mPDF 4.4.003
			null
		...
		return// $path_cmd .= 'h '; // ?? In error - don't close subpath here
		null $path_cmd;
	}
	...
	// fonction retracant les <polygone />
	nullfunction svgPolygon($arguments) {
		$xbase = $arguments [0];
		$ybase = - $arguments [1];
		$path_cmd = sprintf ( '%.3F %.3F m ', $xbase * $this->kp, $ybase * $this->kp );
		for($i = 2; $i < count ( $arguments ); $i += 2) {
	...
	// write string to image
	nullfunction svgText() {
		// $tmp = count($this->txt_style)-1;
		$current_style// $tmp = count($this->txt_style)-1;
		null = array_pop ( $this->txt_style );
		$style = '';
		$render = - 1;
		if (isset ( $this->txt_data [2] )) {
			// select font
			$style// select font
			null .= ($current_style ['font-weight'] == 'bold') ? 'B' : '';
			$style .= ($current_style ['font-style'] == 'italic') ? 'I' : '';
			$size = $current_style ['font-size'] * $this->kf; // mPDF 5.0.039

			// mPDF 5.0
			$current_style// mPDF 5.0.039
			null ['font-family'] = $this->mpdf_ref->SetFont ( $current_style ['font-family'], $style, $size, false );
			$this->mpdf_ref->CurrentFont ['fo'] = true; // mPDF 5.0.039


			// mPDF 5.0.041
			$opacitystr// mPDF 5.0.039
			null = '';
			$opacity = 1;
			...
			$y// mPDF 4.4.003
			null = $this->ConvertSVGSizePixels ( $this->txt_data [1], 'y' ); // mPDF 4.4.003
			$txt// mPDF 4.4.003
			null = $this->txt_data [2];
			
			// mPDF 4.4.003
			$txt// mPDF 4.4.003
			null = preg_replace ( '/\f/', '', $txt );
			...
			// mPDF 5.0
			if// mPDF 5.0
			null ($this->mpdf_ref->usingCoreFont) {
			...
			$this// mPDF 4.4.003
			null->mpdf_ref->magic_reverse_dir ( $txt, true, 'ltr' ); // mPDF 5.0.054
			$this// mPDF 5.0.054
			null->mpdf_ref->ConvertIndic ( $txt );
			...
			null else if ($current_style ['text-anchor'] == 'end') {
			...
			null else
				...
				$txt// mPDF 5.0 adds chars to subset list
				null = $this->mpdf_ref->UTF8ToUTF16BE ( $txt, false );
			}
			...
			// mPDF 5.0.041
			// mPDF 5.0.051
			$path_cmd// mPDF 5.0.051
			null = sprintf ( 'q BT /F%d %s %.3F Tf %.3F %.3F Td %s Tr %s %s %s Tj ET Q ', $this->mpdf_ref->CurrentFont ['i'], $opacitystr, $this->mpdf_ref->FontSizePt, $pdfx * $this->kp, $pdfy * $this->kp, $render, $fillstr, $strokestr, $txt ) . "\n";
			unset ( $this->txt_data [0], $this->txt_data [1], $this->txt_data [2] );
			
			// mPDF 5.4.12
			if// mPDF 5.4.12
			null (isset ( $current_style ['font-size-parent'] )) {
		...
		//		$path_cmd .= 'h ';	// mPDF 5.0
		return// $path_cmd .= 'h '; // mPDF 5.0
		null $path_cmd;
	}
	function svgDefineTxtStyle($critere_style) {
		// get copy of current/default txt style, and modify it with supplied attributes
		$tmp// get copy of current/default txt style, and modify it with supplied attributes
		null = count ( $this->txt_style ) - 1;
		...
		if (isset ( $critere_style ['style'] )) {
			...
			// mPDF 5.0.039
			$tmp// mPDF 5.0.039
			null = preg_replace ( "/(.*)font-family:\s*([a-z0-9.]*|none)(.*)/i", "$2", $critere_style ['style'] );
			if ($tmp != $critere_style ['style']) {
		...
		if (isset ( $critere_style ['font'] )) {
			...
			$tmp// [ [ <'font-style'> || <'font-variant'> || <'font-weight'> ]?<'font-size'> [ / <'line-height'> ]? <'font-family'> ]
			null = preg_replace ( "/(.*)(italic|oblique)(.*)/i", "$2", $critere_style ['font'] );
			...
			$tmp// select digits not followed by percent sign nor preceeded by forward slash
			null = preg_replace ( "/(.*)\b(\d+)[\b|\/](.*)/i", "$2", $critere_style ['font'] );
			if ($tmp != $critere_style ['font']) {
		...
		if (isset ( $critere_style ['font-size'] )) {
			// mPDF 5.4.12
			if// mPDF 5.4.12
			null (strpos ( $critere_style ['font-size'], '%' ) !== false) {
		...
		if (isset ( $critere_style ['font-family'] )) {
			...
			foreach ( $aux_fontlist as $f ) {
				...
				if ((! $this->mpdf_ref->usingCoreFont && in_array ( $v, $this->mpdf_ref->available_unifonts )) || ($this->mpdf_ref->usingCoreFont && in_array ( $v, array (
						'courier',
						'times',
						'helvetica',
						'arial' 
	...
	// add current style to text style array (will remove it later after writing text to svg_string)
		nullarray_push ( $this->txt_style, $current_style );
	}
	
	//
	//	fonction ajoutant un gradient
	// fonction ajoutant un gradient
	nullfunction svgAddGradient($id, $array_gradient) {
	...
	// Ajoute une couleur dans le gradient correspondant
	nullfunction svgWriteString($content) {
		$this->svg_string .= $content;
	}
	
	//	analise le svg et renvoie aux fonctions precedente our le traitement
	// analise le svg et renvoie aux fonctions precedente our le traitement
	nullfunction ImageSVG($data) {
		$this->svg_info = array ();
		
		// mPDF 4.4.006
		if// mPDF 4.4.006
		null (preg_match ( '/<!ENTITY/si', $data )) {
			// Get User-defined entities
			// Get User-defined entities
			nullpreg_match_all ( '/<!ENTITY\s+([a-z]+)\s+\"(.*?)\">/si', $data, $ent );
			// Replace entities
			for// Replace entities
			null($i = 0; $i < count ( $ent [0] ); $i ++) {
		...
		// mPDF 4.4.003
		if// mPDF 4.4.003
		null (preg_match ( '/xlink:href=/si', $data )) {
			// Get links
			// Get links
			nullpreg_match_all ( '/(<(linearGradient|radialgradient)[^>]*)xlink:href=["\']#(.*?)["\'](.*?)\/>/si', $data, $links );
			if (count ( $links [0] )) {
				$links [5] = array ();
			} // mPDF 4.5.010
			                                               // Delete links from data - keeping in $links
			for// Delete links from data - keeping in $links
			null($i = 0; $i < count ( $links [0] ); $i ++) {
				$links [5] [$i] = 'tmpLink' . RAND ( 100000, 9999999 ); // mPDF 4.5.010
				$data// mPDF 4.5.010
				null = preg_replace ( '/' . preg_quote ( $links [0] [$i], '/' ) . '/is', '<MYLINKS' . $links [5] [$i] . '>', $data ); // mPDF 4.5.010
			}// mPDF 4.5.010
			// Get targets
			// Get targets
			nullpreg_match_all ( '/<(linearGradient|radialgradient)([^>]*)id=["\'](.*?)["\'](.*?)>(.*?)<\/(linearGradient|radialgradient)>/si', $data, $m );
			$targets = array ();
			$stops = array ();
			// keeping in $targets
			for// keeping in $targets
			null($i = 0; $i < count ( $m [0] ); $i ++) {
				$stops [$m [3] [$i]] = $m [5] [$i];
			}
			// Add back links this time as targets (gradients)
			for// Add back links this time as targets (gradients)
			null($i = 0; $i < count ( $links [0] ); $i ++) {
				$def = $links [1] [$i] . ' ' . $links [4] [$i] . '>' . $stops [$links [3] [$i]] . '</' . $links [2] [$i] . '>'; // mPDF 4.5.010
				$data// mPDF 4.5.010
				null = preg_replace ( '/<MYLINKS' . $links [5] [$i] . '>/is', $def, $data ); // mPDF 4.5.010
			}// mPDF 4.5.010
			null
		}
		// mPDF 4.4.003	- Removes <pattern>
		$data// mPDF 4.4.003 - Removes <pattern>
		null = preg_replace ( '/<pattern.*?<\/pattern>/is', '', $data );
		// mPDF 4.4.003	- Removes <marker>
		$data// mPDF 4.4.003 - Removes <marker>
		null = preg_replace ( '/<marker.*?<\/marker>/is', '', $data );
		...
		if// chargement unique des fonctions
		null (! function_exists ( "xml_svg2pdf_start" )) { // mPDF 5.3.76

			// mPDF 5.3.76
			nullfunction xml_svg2pdf_start($parser, $name, $attribs) {
				//
				//	definition
				global// definition
				null $svg_class, $last_gradid;
				
				// mPDF 4.4.003
				$svg_class// mPDF 4.4.003
				null->xbase = 0;
				$svg_class->ybase = 0;
				switch (strtolower ( $name )) {
					
					// mPDF 5.0.039	- Don't output stuff inside <defs>
				case// mPDF 5.0.039 - Don't output stuff inside <defs>
					null 'defs' :
					...
					case 'path' :
						...
						// mPDF 5.6.65
					// mPDF 5.6.65
						nullpreg_match_all ( '/([MZLHVCSQTAmzlhvcsqta])([eE ,\-.\d]+)*/', $path, $commands, PREG_SET_ORDER );
						$path_cmd = '';
						$svg_class->subPathInit = true;
						// mPDF 5.0.039
					$svg_class// mPDF 5.0.039
						null->pathBBox = array (
								999999,
								999999,
								- 999999,
						...
						// mPDF 5.0.039
					if// mPDF 5.0.039
						null ($svg_class->pathBBox [2] == - 1999998) {
					...
					$critere_style// definition du style de la forme:
						null = $attribs;
						unset ( $critere_style ['points'] );
						$path_style = $svg_class->svgDefineStyle ( $critere_style );
						break;
					...
					case 'stop' :
						...
						if// mPDF 4.4.003 // mPDF 5.0.040
						null (isset ( $attribs ['style'] ) and preg_match ( '/stop-color:\s*([^;]*)/i', $attribs ['style'], $m )) {
							$color = trim ( $m [1] );
						...
						// mPDF 5.3.74
						if// mPDF 5.3.74
						null ($col {0} == 3 || $col {0} == 5) { // RGB
							$color_final// RGB
							null = sprintf ( '%.3F %.3F %.3F', ord ( $col {1} ) / 255, ord ( $col {2} ) / 255, ord ( $col {3} ) / 255 );
							$svg_class->svg_gradient [$last_gradid] ['colorspace'] = 'RGB';
						} else if ($col {0} == 4 || $col {0} == 6) { // CMYK
							$color_final// CMYK
							null = sprintf ( '%.3F %.3F %.3F %.3F', ord ( $col {1} ) / 100, ord ( $col {2} ) / 100, ord ( $col {3} ) / 100, ord ( $col {4} ) / 100 );
							$svg_class->svg_gradient [$last_gradid] ['colorspace'] = 'CMYK';
						} else if ($col {0} == 1) { // Grayscale
							$color_final// Grayscale
							null = sprintf ( '%.3F', ord ( $col {1} ) / 255 );
							$svg_class->svg_gradient [$last_gradid] ['colorspace'] = 'Gray';
						}
						
						// mPDF 5.0.020
						$stop_opacity// mPDF 5.0.020
						null = 1;
						// mPDF 4.4.003
						if// mPDF 4.4.003
						null (isset ( $attribs ['style'] ) and preg_match ( '/stop-opacity:\s*([0-9.]*)/i', $attribs ['style'], $m )) {
							$stop_opacity = $m [1];
					...
					case 'a' :
						...
						}// not handled like a xlink:href in other elements // then continue like a <g>
				case// then continue like a <g>
					null 'g' :
					...
					case 'text' :
						// mPDF 4.4.003
						$array_style// mPDF 4.4.003
						null = $svg_class->svgDefineStyle ( $attribs );
						if ($array_style ['transformations']) {
							$svg_class->svgWriteString ( ' q ' . $array_style ['transformations'] );
						}
						array_push ( $svg_class->svg_style, $array_style );
				...
				if// insertion des path et du style dans le flux de donn� general.
				null (isset ( $path_cmd ) && $path_cmd) { // mPDF 4.4.003
				                                     // mPDF 5.0
					list// mPDF 5.0
					null ( $prestyle, $poststyle ) = $svg_class->svgStyle ( $path_style, $attribs, strtolower ( $name ) );
					if ($path_style ['transformations']) { // transformation on an element
						$svg_class// transformation on an element
						null->svgWriteString ( " q " . $path_style ['transformations'] . " $prestyle $path_cmd $poststyle" . " Q\n" );
					} else {
			...
			function xml_svg2pdf_end($parser, $name) {
				...
				switch ($name) {
					...
					case "text" :
						...
						$svg_class// echo "style >> ".$get_style[1]."<br><br>";
						null->svgWriteString ( $path_cmd );
						// mPDF 4.4.003
						$tmp// mPDF 4.4.003
						null = count ( $svg_class->svg_style ) - 1;
						$current_style = $svg_class->svg_style [$tmp];
						if ($current_style ['transformations']) {
							$svg_class->svgWriteString ( " Q\n" );
						}
				...
				// mPDF 5.0.039	- Don't output stuff inside <defs>
				if// mPDF 5.0.039 - Don't output stuff inside <defs>
				null ($name == 'defs') {
		...
		$svg_class = $this;
		// mPDF 5.0.039	- Don't output stuff inside <defs>
		$svg_class// mPDF 5.0.039 - Don't output stuff inside <defs>
		null->inDefs = false;
...
// mPDF 5.0.040
// END OF CLASS
nullfunction calc_bezier_bbox($start, $c) {
	$P0 = array (
			$start [0],
			$start [1] 
	);
...
// mPDF 5.0.040
// mPDF 5.0.040
nullfunction _testIntersectCircle($cx, $cy, $cr) {
	...
	if// If centre of circle is inside 0,0,1,1 square
	null ($cx >= 0 && $cx <= 1 && $cy >= 0 && $cy <= 1) {
		$maxd = 1.5;
