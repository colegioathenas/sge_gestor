...
if (! defined ( '__CLASS_HTML2PDF__' )) {
	...
	class HTML2PDF {
		 ...
		 *        	page orientation, same as TCPDF
		 * @param mixed $format
		 *        	The format used for pages, same as TCPDF
		 * @param $tring $langue
		 *        	Langue : fr, en, it...
		 ...
		 */
		public function __construct($orientation = 'P', $format = 'A4', $langue = 'fr', $unicode = true, $encoding = 'UTF-8', $marges = array(5, 5, 5, 20)) {
		 ...
		 *        	name of the default font to use. If null : Arial is no font is specify, and error if the asked font does not exist
		 * @return HTML2PDF $this
		 */
		 ...
		 *        	index title
		 * @param int $sizeTitle
		 *        	font size of the index title, in mm
		 * @param int $sizeBookmark
		 *        	font size of the index, in mm
		 * @param boolean $bookmarkTitle
		 *        	add a bookmark for the index, at his beginning
		 * @param boolean $displayPage
		 *        	display the page numbers
		 * @param int $onPage
		 *        	if null : at the end of the document on a new page, else on the $onPage page
		 ...
		 * @access public
		 * @param string $html        	
		 * @param boolean $debugVue
		 *        	enable the HTML debug vue
		 * @return null
		 */
		...
		/**
		 * init a sub HTML2PDF.
		 * does not use it directly. Only the method createSubHTML must use it
		 *
		 * @access public
		 * @param string $format        	
		 * @param string $orientation        	
...
' . $content . '
		 ...
		 *        	(mm, top margin)
		 * @param int $right
		 *        	(mm, right margin, if null => left=right)
		 * @param int $bottom
		 *        	(mm, bottom margin, if null => bottom=8mm)
		 */
		 ...
		 * @param integer $curr
		 *        	real position in the html parseur (if break line in the write of a text)
		 ...
		 *        	step name
		 * @param boolean $level
		 *        	(true=up, false=down, null=nothing to do)
		 * @return $this
		 */
		...
		protected function _saveMargin($ml, $mt, $mr) {
			...
			$this->_pageMarges = array ();
			$this->_pageMarges [floor ( $mt * 100 )] = array (
					$ml,
					$this->pdf->getW () - $mr 
			);
		}
		 ...
		 * @param float $h        	
		 * @param integer $curr
		 *        	real current position in the text, if new line in the write of a text
		 */
		...
		protected function _prepareSubObj() {
			...
			// create the sub object
			HTML2PDF::$_subobj = new HTML2PDF ( $this->_orientation, $this->_format, $this->_langue, $this->_unicode, $this->_encoding, array (
					$this->_defaultLeft,
					$this->_defaultTop,
					$this->_defaultRight,
					$this->_defaultBottom 
		 ...
		 *
		 * @access protected
		 * @param &HTML2PDF $subHtml
		 *        	sub HTML2PDF to create
		 * @param integer $cellmargin
		 *        	if in a TD : cellmargin of this td
		 */
		...
		protected function _listeAddLevel($type = 'ul', $style = '', $img = null) {
			...
			// add the new level
			$this->_defList [count ( $this->_defList )] = array (
					'style' => $style,
					'nb' => 0,
					'img' => $img 
			);
		...
		protected function _drawImage($src, $subLi = false) {
			...
			if (! $this->_subPart && ! $this->_isSubPart) {
				if ($src) {
					...
					$this->pdf->setFillColorArray ( array (
							240,
							220,
							220 
					) );
		 ...
		 * @param array $border        	
		 * @param float $padding
		 *        	- internal marge of the rectanble => not used, but...
		 * @param float $margin
		 *        	- external marge of the rectanble
		 * @param array $background        	
		 * @return boolean
		 */
		protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background) {
			...
			if ($background ['image']) {
				...
				$iPosition = $background ['position'] !== null ? $background ['position'] : array (
						0,
						0 
				);
				$iRepeat = $background ['repeat'] !== null ? $background ['repeat'] : array (
						true,
						true 
				);
		 ...
		 *        	(binary from 0 to 3 with 1=>start with a radius, 2=>end with a radius)
		 */
		 ...
		 * @param array $param        	
		 * @param string $other
		 *        	name of tag that used the div tag
		 * @return boolean
		 */
		...
		protected function _tag_open_BARCODE($param) {
			...
			if ($this->_testIsDeprecated && (isset ( $param ['bar_h'] ) || isset ( $param ['bar_w'] )))
				throw new HTML2PDF_exception ( 9, array (
						'BARCODE',
						'bar_h, bar_w' 
				) );
		...
		protected function _tag_open_WRITE($param) {
			...
			if (in_array ( $this->parsingCss->value ['id_tag'], array (
					'fieldset',
					'legend',
					'div',
					'table',
			...
			while ( $x + $w > $right && $x < $right + $space && count ( $words ) ) {
				...
				if (count ( $words )) {
					...
					if ($nb > 10000) {
						...
						foreach ( $words as $k => $word )
							$txt .= ($k ? ' ' : '') . $word [0];
						throw new HTML2PDF_exception ( 2, array (
								$txt,
								$right - $left,
								$w 
						) );
		...
		protected function _tag_open_A($param) {
			...
			$this->parsingCss->save ();
			$this->parsingCss->value ['font-underline'] = true;
			$this->parsingCss->value ['color'] = array (
					20,
					20,
					250 
			);
		...
		protected function _tag_open_P($param) {
			...
			if (! in_array ( $this->_previousCall, array (
					'_tag_close_P',
					'_tag_close_UL' 
			) )) {
				if ($this->_maxH)
		...
		protected function _tag_open_TD($param, $other = 'td') {
			...
			if ($this->_subPart) {
				...
				for($j = 0; $j < $rowspan; $j ++) {
					for($i = 0; $i < $colspan; $i ++) {
						HTML2PDF::$_tables [$param ['num']] ['corr'] [HTML2PDF::$_tables [$param ['num']] ['corr_y'] + $j] [HTML2PDF::$_tables [$param ['num']] ['corr_x'] + $i] = ($i + $j > 0) ? '' : array (
								$x,
								$y,
								$colspan,
								$rowspan 
		...
		protected function _tag_close_TD($param) {
			...
			if ($this->_subPart) {
				...
				HTML2PDF::$_tables [$param ['num']] ['cases'] [HTML2PDF::$_tables [$param ['num']] ['tr_curr'] - 1] [HTML2PDF::$_tables [$param ['num']] ['td_curr'] - 1] ['w'] = max ( array (
						$w0,
						$w2 
				) );
				HTML2PDF::$_tables [$param ['num']] ['cases'] [HTML2PDF::$_tables [$param ['num']] ['tr_curr'] - 1] [HTML2PDF::$_tables [$param ['num']] ['td_curr'] - 1] ['h'] = max ( array (
						$h0,
						$h2 
				) );
		...
		/**
		 * new page for the automatic Index, does not use thie method.
		 * Only HTML2PDF_myPdf could use it !!!!
		 *
		 * @param &int $page        	
