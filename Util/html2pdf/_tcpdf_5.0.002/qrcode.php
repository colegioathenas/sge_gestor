...
if (! class_exists ( 'QRcode', false )) {
	...
	class QRcode {
		 ...
		 */
		// -------------------------------------------------
		nullpublic function __construct($code, $eclevel = 'L') {
			$barcode_array = array ();
			if ((is_null ( $code )) or ($code == '\0') or ($code == '')) {
				return false;
			}
			// set error correction level
			$this// set error correction level
			null->level = array_search ( $eclevel, array (
					'L',
					'M',
					'Q',
					'H' 
		...
		protected function binarize($frame) {
			...
			foreach// the frame is square (width = height)
			null ( $frame as &$frameLine ) {
				for($i = 0; $i < $len; $i ++) {
		...
		protected function encodeMask($mask) {
			...
			for// inteleaved data and ecc codes
			null($i = 0; $i < ($this->dataLength + $this->eccLength); $i ++) {
				$code = $this->getCode ();
				$bit = 0x80;
				for($j = 0; $j < 8; $j ++) {
			...
			// remainder bits
			$j// remainder bits
			null = $this->getRemainder ( $this->version );
			for($i = 0; $i < $j; $i ++) {
				$addr = $this->getNextPosition ();
			...
			// masking
			$this// masking
			null->runLength = array_fill ( 0, QRSPEC_WIDTH_MAX + 1, 0 );
			if ($mask < 0) {
		 ...
		 */
		// - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function setFrameAt($at, $val) {
		 ...
		 */
		// - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function init($spec) {
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function writeFormatInformation($width, &$frame, $mask, $level) {
			$blacks = 0;
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function isdigitat($str, $pos) {
			if ($pos >= strlen ( $str )) {
				return false;
			}
		...
		protected function eatNum() {
			...
			if ($mode == QR_MODE_8B) {
				...
				- $this// + 4 + l8
				null->estimateBitsMode8 ( $run + 1 ); // - 4 - l8
				if// - 4 - l8
				null ($dif > 0) {
					return $this->eat8 ();
				}
			...
			if ($mode == QR_MODE_AN) {
				...
				- $this// + 4 + la
				null->estimateBitsModeAn ( $run + 1 ); // - 4 - la
				if// - 4 - la
				null ($dif > 0) {
					return $this->eatAn ();
				}
		...
		protected function eatAn() {
			...
			while ( $this->isalnumat ( $this->dataStr, $p ) ) {
				if ($this->isdigitat ( $this->dataStr, $p )) {
					...
					+ $this// + 4 + la
					null->estimateBitsModeNum ( $q - $p ) + 4 + $ln - $this->estimateBitsModeAn ( $q ); // - 4 - la
					if// - 4 - la
					null ($dif < 0) {
						break;
					} else {
		...
		protected function eat8() {
			...
			while ( $p < $dataStrLen ) {
				...
				if ($mode == QR_MODE_NM) {
					...
					+ $this// + 4 + l8
					null->estimateBitsModeNum ( $q - $p ) + 4 + $ln - $this->estimateBitsMode8 ( $q ); // - 4 - l8
					if// - 4 - l8
					null ($dif < 0) {
						break;
					} else {
					...
					$dif = $this->estimateBitsMode8 ( $p ) + 					// + 4 + l8
					+ $this// + 4 + l8
					null->estimateBitsModeAn ( $q - $p ) + 4 + $la - $this->estimateBitsMode8 ( $q ); // - 4 - l8
					if// - 4 - l8
					null ($dif < 0) {
						break;
					} else {
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function newInputItem($mode, $size, $data, $bstream = null) {
		 ...
		 */
		// - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function appendNewInputItem($items, $mode, $size, $data) {
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function allocate($setLength) {
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function qrstrset($srctab, $x, $y, $repl, $replLen = false) {
		...
		protected function createFrame($version) {
			...
			// Finder pattern
			$frame// Finder pattern
			null = $this->putFinderPattern ( $frame, 0, 0 );
			...
			// Format info
			$setPattern// Format info
			null = str_repeat ( "\x84", 9 );
			...
			null = $this->putAlignmentPattern ( $version, $frame, $width );
			// Version information
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function init_rs($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
		 ...
		 */
		 // - - - - - - - - - - - - - - - - - - - - - - - - -
		nullprotected function modnn($rs, $x) {
			while ( $x >= $rs ['nn'] ) {
				$x -= $rs ['nn'];
				$x = ($x >> $rs ['mm']) + ($x & $rs ['nn']);
			}
		...
		protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
			...
			$rs// Based on Reed solomon encoder by Phil Karn, KA9Q (GNU-LGPLv2)
			null = null;
			// Check parameter ranges
			if// Check parameter ranges
			null (($symsize < 0) or ($symsize > 8)) {
				return $rs;
			}
			...
			$NN// PHP style macro replacement ;)
			null = & $rs ['nn'];
			$A0 = & $NN;
			// Generate Galois field lookup tables
			$rs// Generate Galois field lookup tables
			null ['index_of'] [0] = $A0; // log(zero) = -inf
			$rs// log(zero) = -inf
			null ['alpha_to'] [$A0] = 0; // alpha**-inf = 0
			$sr// alpha**-inf = 0
			null = 1;
			for($i = 0; $i < $rs ['nn']; ++ $i) {
				$rs ['index_of'] [$sr] = $i;
			...
			if ($sr != 1) {
				...
				return// field generator polynomial is not primitive!
				null NULL;
			}
			// Form RS code generator polynomial from its roots
			$rs// Form RS code generator polynomial from its roots
			null ['genpoly'] = array_fill ( 0, ($nroots + 1), 0 );
			$rs ['fcr'] = $fcr;
			$rs ['prim'] = $prim;
			$rs ['nroots'] = $nroots;
			$rs ['gfpoly'] = $gfpoly;
			// Find prim-th root of 1, used in decoding
			for// Find prim-th root of 1, used in decoding
			null($iprim = 1; ($iprim % $prim) != 0; $iprim += $rs ['nn']) {
			...
			for($i = 0, $root = $fcr * $prim; $i < $nroots; $i ++, $root += $prim) {
				...
				for// Multiply rs->genpoly[] by @**(root + x)
				null($j = $i; $j > 0; -- $j) {
					if ($rs ['genpoly'] [$j] != 0) {
				...
				$rs// rs->genpoly[0] can never be zero
				null ['genpoly'] [0] = $rs ['alpha_to'] [$this->modnn ( $rs, $rs ['index_of'] [$rs ['genpoly'] [0]] + $root )];
			}
			// convert rs->genpoly[] to index form for quicker encoding
			for// convert rs->genpoly[] to index form for quicker encoding
			null($i = 0; $i <= $nroots; ++ $i) {
		...
		protected function encode_rs_char($rs, $data, $parity) {
			...
			$NN// bits per symbol
			null = & $rs ['nn']; // the total number of symbols in a RS block
			$ALPHA_TO// the total number of symbols in a RS block
			null = & $rs ['alpha_to']; // the address of an array of NN elements to convert Galois field elements in index (log) form to polynomial form
			$INDEX_OF// the address of an array of NN elements to convert Galois field elements in index (log) form to polynomial form
			null = & $rs ['index_of']; // the address of an array of NN elements to convert Galois field elements in polynomial form to index (log) form
			$GENPOLY// the address of an array of NN elements to convert Galois field elements in polynomial form to index (log) form
			null = & $rs ['genpoly']; // an array of NROOTS+1 elements containing the generator polynomial in index form
			$NROOTS// an array of NROOTS+1 elements containing the generator polynomial in index form
			null = & $rs ['nroots']; // the number of roots in the RS code generator polynomial, which is the same as the number of parity symbols in a block
			$FCR// the number of roots in the RS code generator polynomial, which is the same as the number of parity symbols in a block
			null = & $rs ['fcr']; // first consecutive root, index form
			$PRIM// first consecutive root, index form
			null = & $rs ['prim']; // primitive element, index form
			$IPRIM// primitive element, index form
			null = & $rs ['iprim']; // prim-th root of 1, index form
			$PAD// prim-th root of 1, index form
			null = & $rs ['pad']; // the number of pad symbols in a block
			$A0// the number of pad symbols in a block
			null = & $NN;
			...
			for($i = 0; $i < ($NN - $NROOTS - $PAD); $i ++) {
				...
				if ($feedback != $A0) {
					...
					$feedback// always be for the polynomials constructed by init_rs()
					null = $this->modnn ( $rs, $NN - $GENPOLY [$NROOTS] + $feedback );
					for($j = 1; $j < $NROOTS; ++ $j) {
...
}// end QRcode class // END OF "class_exists QRcode"
