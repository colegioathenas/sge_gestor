...
class HTML2PDF_parsingCss {
	 ...
	 *
	 * @param
	 *        	&HTML2PDF_myPdf reference to the PDF $object
	 * @access public
	 */
	 ...
	 *
	 * @param
	 *        	&HTML2PDF reference to the HTML2PDF parent $object
	 * @access public
	 */
	 ...
	 *
	 * @param
	 *        	string default font-family. If null : Arial for no font asked, and error fot ont does not exist
	...
	public function analyse($tagName, &$param, $legacy = null) {
		...
		foreach ( $styles as $nom => $val ) {
			switch ($nom) {
				...
				case 'border-top-left-radius' :
					...
					$this->value ['border'] ['radius'] ['tl'] = array (
							$val [0],
							isset ( $val [1] ) ? $val [1] : $val [0] 
					);
					break;
				...
				case 'border-top-right-radius' :
					...
					$this->value ['border'] ['radius'] ['tr'] = array (
							$val [0],
							isset ( $val [1] ) ? $val [1] : $val [0] 
					);
					break;
				...
				case 'border-bottom-right-radius' :
					...
					$this->value ['border'] ['radius'] ['br'] = array (
							$val [0],
							isset ( $val [1] ) ? $val [1] : $val [0] 
					);
					break;
				...
				case 'border-bottom-left-radius' :
					...
					$this->value ['border'] ['radius'] ['bl'] = array (
							$val [0],
							isset ( $val [1] ) ? $val [1] : $val [0] 
					);
					break;
		...
		if ($noWidth && in_array ( $tagName, array (
				'div',
				'blockquote',
				'fieldset' 
		) ) && $this->value ['position'] != 'absolute') {
	 ...
	 * @access public
	 * @param boolean $mode
	 *        	true => adding padding and border
	 ...
	 * @access protected
	 * @param string $key
	 *        	CSS selector to analyse
	 * @param array $lst
	 *        	list of the selectors of each tags
	 * @param string $next
	 *        	next step of parsing the selector
	 * @return boolean
	 */
	 ...
	 * @param &array $value
	 *        	parsed values (by reference, because, ther is a legacy of the parent CSS properties)
	 */
