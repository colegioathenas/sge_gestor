...
foreach ( $cabecalho as $aluno ) {
	...
	foreach ( $disciplinas_registro as $disciplina ) {
		...
		if ($notas [$divisao ["nCdDivisao"]] [$disciplina ["nCdDisciplina"]] ["nMedia"] < 5) {
			if ($resultado_final != "<b>Reprovado</b>") {
				$resultado_final = "<b>Reprovado</b>";
				if ($resultado_final != "<b>Dependencia</b>") {
					// verifica conselho
					$query = "select * from conselho_final where nCdMatricula = $nCdUsuario and nCdDisciplina = " . $disciplina ["nCdDisciplina"];
					$resultado_consulta = consulta ( "athenas", $query );
					if ($resultado_consulta [0] ["cConceito"] == "C") {
						$media = "5.00";
						$resultado_final = "<b>Aprov. Conselho</b>";
					}
					if ($resultado_consulta [0] ["cConceito"] == "D") {
						$resultado_final = "<b>Dependencia</b>";
					}
				}
			}
		}
